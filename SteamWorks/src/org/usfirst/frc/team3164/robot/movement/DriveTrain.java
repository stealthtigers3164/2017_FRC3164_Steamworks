...
public class DriveTrain<T extends BasicMotor> extends MotorSet<T> {
	 ...
	 */
	public void forzaDrive(double axisX, double axisY, boolean slowmode) {
	...
	public void updateMotors() {
		...
		if (shouldUpdate()) {
			...
			case FORZA_DRIVE:
				...
				forzaDrive(gamePad.sticks.RIGHT_X.getScaled(), gamePad.sticks.LEFT_Y.getScaled(), (gamePad.buttons.BUTTON_LB.isOn()));
				break;
			default:
				//forzaDrive(gamePad.sticks.LEFT_X.getScaled(), gamePad.trigger.getAxis());
				forzaDrive(gamePad.sticks.RIGHT_X.getScaled(), gamePad.sticks.LEFT_Y.getScaled(), (gamePad.buttons.BUTTON_LB.isOn()));
