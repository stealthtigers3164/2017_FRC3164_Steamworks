...
public class Vision {
	...
	public static void main(String[] args) {
		...
		NetworkTable tableDist = networkTables.getTable("grip");
		...
		while(true) {
			...
			if(camera.retrieve(img)) {
				...
				if(bk) {
		            ...
		            float leftEdgesRatio = (float) (209.55 / (boxRightX - boxLeftX + 0.001)); // mm per pixel
		            float realDistFromCenter = (float) ((320.0 - 0.5 * (boxLeftX + (boxRightX + boxRightW))) * leftEdgesRatio);
		                // Image Center Point (320) - Box Center Point (average of left and right bound)
		                float forwardDist = (float) tableDist.getNumber("lidar");
		                float turnAngle = (float) Math.toDegrees(Math.atan(realDistFromCenter / forwardDist));

		                String turnDirection = "center";
		                int deadzone = 5;  // deadzone to determine if center, in degrees
		                if(turnAngle > deadzone) {
		                    turnDirection = "left";
		                } else if (turnAngle < (0 - deadzone)) {
		                    turnDirection = "right";
	                    }
		                table.putBoolean("boxVisible", true);
		                table.putNumber("distLeft", boxLeftX);
		                ...
		                table.putNumber("distFromCenterPoint", realDistFromCenter);
		                table.putNumber("turnAngle", turnAngle);
		                table.putString("turnDirection", turnDirection);
